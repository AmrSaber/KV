...
func TestRenameCrossDB(t *testing.T) {
	t.Run("basic rename across DBs", func(t *testing.T) {
		SetupTestDB(t)
		RunKVSuccess(t, "set", "k@db1", "value")

		// Same-DB rename
		RunKVSuccess(t, "rename", "k@db1", "same-db@db1")
		...
		// Cross-DB rename
		RunKVSuccess(t, "rename", "same-db@db1", "moved@db2")
		...
		RunKVSuccess(t, "rename", "tmp@db2", "@db3")
	...
	t.Run("rename preserves history+lock+TTL across DBs", func(t *testing.T) {
		...
		// TTL preserves (rename preserves TTL, unlike copy)
	...
	t.Run("rename to existing key fails across DBs", func(t *testing.T) {
		SetupTestDB(t)
		RunKVSuccess(t, "set", "existing@db2", "occupied")
		RunKVSuccess(t, "set", "source@db1", "value")
		output := RunKVFailure(t, "rename", "source@db1", "existing@db2")
